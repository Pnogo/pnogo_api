...
from pnogo_api.db import execute_db, query_db
from pnogo_api.s3 import delete_object, get_object, put_object
...
def getall_all():
    pnogos = query_db(
        "SELECT p.id, p.file, p.description, p.points, p.sent, p.daily_date, c.name FROM pictures p JOIN cndr c ON p.cndr_id=c.id",
        multi=True,
    )
...
def getall(cndr):
    pnogos = query_db(
        "SELECT p.id, p.file, p.description, p.points, p.sent, CAST(p.daily_date AS TEXT) FROM pictures p JOIN cndr c ON p.cndr_id=c.id WHERE c.name LIKE %s",
        [escape(cndr)],
        multi=True,
    )
...
def descpnogo():
    ...
    execute_db(
        "UPDATE pictures SET description = %s WHERE id = %s",
        (
            desc,
            pnid,
        ),
...
def infopnogo():
    pnid = request.args.get("id")
    pongo = query_db(
        "SELECT p.file, p.description, p.points, p.sent, CAST(p.daily_date AS TEXT), c.name FROM pictures p JOIN cndr c ON p.cndr_id=c.id WHERE p.id = %s",
        [pnid],
    )
    return (
        {
...
def random_cndr(cndr):
    pongo = query_db(
        "SELECT id, description, points FROM pictures WHERE id IN (SELECT p.id FROM pictures p JOIN cndr c ON cndr_id=c.id WHERE c.name ILIKE %s ORDER BY RANDOM() LIMIT 1)",
        [escape(cndr)],
    )
    return (
        {
...
def daily():
    ...
    if pnid is not None:
        ...
        pongo = query_db(
            "SELECT p.id, p.description, p.points, c.name FROM pictures p JOIN cndr c ON cndr_id=c.id WHERE p.id=%s",
            pnid,
        )
        return {
            "id": pongo[0],
